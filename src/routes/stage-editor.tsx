    ...
    <div className='flex w-full'>
      ...
      <main className="overflow-hidden w-7/12 flex flex-col border-r">
        <header className='flex justify-between p-2'>
          ...
          <div className='flex gap-2'>
            <Button type="button" size="sm" variant="ghost" onClick={() => sgDialog.current?.show()}>Add Stage Group</Button>
            <Button type="button" size="sm" variant="secondary" onClick={toggleEditorWindow}>Show Board</Button>

            {isEditing && isEditing.id !== "new" ? (<Button variant="destructive" size="sm" type="button" onClick={async () => {
      ...
      <aside className="flex flex-col overflow-hidden w-5/12 p-2">
        <header className='flex w-full pb-2 gap-1'>
          ...
          <Link to="/stage-editor/new" className={buttonVariants({ variant: "secondary" })}>New</Link>
        </header>

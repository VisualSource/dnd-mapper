...
} from "../components/dialog/EntityControlDialog";
import {
...
const ControlPanel: React.FC = () => {
	...
	return (
		<div className="flex h-full w-full">
			...
			<StageSelectionDialog
				ref={ssdialogRef}
				onSelect={(id) => navigate({ to: "/control/$id", params: { id } })}
			/>
			...
			<section className="w-8/12 flex flex-col border-r">
				...
				<ul
					...
					onClick={(ev) => {
						const target = (ev.nativeEvent.target as HTMLElement).closest(
							"li[data-id]",
						);
						if (!target) return;
				...
				>
					{queue.map((e, i) => (
						<li
							key={e.instanceId}
							data-id={e.instanceId}
							className={cn("p-2 border rounded-sm", {
								"opacity-35": !e.entity.displayOnMap,
								"bg-zinc-900": e.entity.displayOnMap && i === 0,
							})}
						>
								...
								<div>
									...
									{!e.entity.isPlayerControlled ? (
										<div className="text-muted-foreground text-left">
											...
											<span
												className={cn({
													"text-blue-400": e.entity.tempHealth > 0,
												})}
											>
												{e.entity.health + e.entity.tempHealth}
											</span>
											/{e.entity.maxHealth}
										</div>
									) : (
			...
			<aside className="w-4/12 flex flex-col gap-4 p-2">
				...
				<div className="flex w-full gap-2">
					<Button type="button" onClick={displayWindowToggle}>
						Toggle Map Window
					</Button>
					<Link
				...
				<div className="flex gap-2 w-full">
					<Button
						...
						variant="outline"
						className="w-full"
						onClick={() => aedialogRef.current?.show()}
					>
						Add Entity
					</Button>
					<Button
				...
				<Button
					type="button"
					variant="secondary"
					onClick={() => ssdialogRef.current?.show()}
				>
					Goto Stage
				</Button>
			</aside>

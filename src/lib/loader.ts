...
export const resloveStage = async (id: string): Promise<ResolvedStage | null> => {
    ...
    for (const instance of stage.entities) {
        const entity = entities.find(e => e?.id === instance.id) as Entity | undefined;
        if (!entity) continue;

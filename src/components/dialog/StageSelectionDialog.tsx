...
export type StageSelectionDialogHandle = {
	show: () => void;
	close: () => void;
};

export const StageSelectionDialog = forwardRef<
	StageSelectionDialogHandle,
...
>(({ onSelect }, ref) => {
	...
	return (
		<dialog
			ref={dialogRef}
			className="bg-background text-foreground backdrop:opacity-70 backdrop:bg-gray-600 shadow-md"
		>
			...
			<div className="max-h-56">
				<StageList
					onClick={(stage) => onSelect(stage.id)}
					filter={query}
					group={group}
				/>
			</div>

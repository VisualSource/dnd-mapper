...
export const AddEntityForm: React.FC<{
	btnMsg?: string;
	resetOnSubmit?: boolean;
	onSubmit: (entity: Entity) => void;
	entity?: Entity;
}> = ({ onSubmit, entity, btnMsg = "Ok" }) => {
	const form = useForm({
			...
			>
				<FormField
					...
					rules={{
						...
						maxLength: {
							message: "Max name length is 256 characters",
							value: 256,
						},
						minLength: {
							message: "Name must be more then 3 characters",
							value: 3,
						},
					}}
					name="name"
					render={({ field }) => (
						<FormItem>
							...
							<FormDescription>A name to discribe the entity</FormDescription>
							<FormMessage />
						</FormItem>
					)}
				/>
				...
				<FormField
					...
					rules={{
						required: {
							message: "A initiative value is required",
							value: true,
						},
					}}
					name="initiative"
					...
					render={({ field }) => (
						<FormItem className="flex flex-row items-center justify-between border p-4">
							<div className="space-y-0.5">
								...
								<FormDescription>
									Show this entity be shown on the map?
								</FormDescription>
							</div>
							<FormControl>
								<Switch
				...
				{!isPlayerControlled ? (
					<div className="flex flex-col gap-2 animate-in fade-in-5">
						<FormField
							...
							rules={{
								required: {
									message: "A health value is required",
									value: true,
								},
							}}
							name="health"
							render={({ field }) => (
								<FormItem>
									...
									<FormDescription>The health of the unit.</FormDescription>
									<FormMessage />
								</FormItem>
							)}
						/>
						<FormField
							...
							rules={{
								required: {
									message: "A max health value is required",
									value: true,
								},
							}}
							name="maxHealth"
							render={({ field }) => (
								<FormItem>
									...
									<FormDescription>The max health of the unit.</FormDescription>
									<FormMessage />
								</FormItem>
							)}
						/>
						<FormField
							...
							rules={{
								required: {
									message: "A temp health value is required",
									value: true,
								},
							}}
							name="tempHealth"

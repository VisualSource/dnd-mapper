...
export const EntityList: React.FC<{
	...
	return (
		<ul
			className="p-2 space-y-2 overflow-y-scroll h-full"
			onClick={(ev) => {
		...
		>
			{data.length > 0 ? (
				data.map((e) => (
					<li
						key={e.id}
						className="flex w-ful shadow bg-card border rounded-lg text-card-foreground"
						data-id={e.id}
					>
						<button
							type="button"
				...
				<li className="w-full text-center p-2">No Entities</li>
			)}
		</ul>
	);
};

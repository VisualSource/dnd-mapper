...
export const StageList: React.FC<{ deletable?: boolean, filter?: string, group?: string, onClick: (stage: Stage) => void }> = ({ deletable, filter, group, onClick }) => {
    ...
    if (!stages) {
        return (
            <div className="h-full w-full text-center">Loading Stages</div>
        );
    }
    ...
    return (
        <ul onClick={(ev) => {
            ...
            {stages.map(e => (
                <li key={e.id} data-id={e.id} className='flex w-full p-2 shadow bg-card border rounded-lg text-card-foreground'>
                    <button type="button" className="flex gap-2 w-full">
                        ...
                        <div>
                            <h1>{e.name}</h1>
                            <p className="text-sm text-muted-foreground text-left">Group: {e.stageGroup}</p>
                        </div>
                    </button>
                    {deletable ? (
                        <div className="flex flex-col justify-center">
                            <Button data-no-load variant="destructive" size="sm" type="button" onClick={async () => {
